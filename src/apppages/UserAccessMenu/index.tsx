...
const UserAccessMenu = () => {
    ...
    // helpers for boolean string
    const isBoolStr = (v: any) => v === "True" || v === "False" || v === "true" || v === "false";
    const getEligibleChecked = (rows: any[], col: string) => {
        let eligible = 0;
        let checked = 0;
        for (const r of rows) {
            const v = r?.[col];
    ...
    // select all per column (only for eligible rows)
    ...
    // XML builder
    const buildXml = (rs0: any[], rs1: any[], tempUserId: string, userType: string) => {
        const master = rs0?.[0]
